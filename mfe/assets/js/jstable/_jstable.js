...
class JSTable {
    ...
    constructor(element, config = {}) {
        ...
        console.warn(this.config.serverSide)
    ...
    _fetchData() {
        ...
        return fetch(query, {
            ...
            that.filteredDataCount = json["@odata.count"];
            return json.value;
        }).then(function (data) {
            let rows = [];
            // Create Table
...
class JSTableRow {
    ...
    static createFromData(recordData) {
        let tr = document.createElement("tr");

        for (const propName in recordData)
        {
            let td = document.createElement('td');
            td.innerHTML = recordData[propName];
            tr.appendChild(td);
        }
